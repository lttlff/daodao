...
public class StringHandler {
	...
	//批量插入keys add by sunJiaming 20140417
	public void setKeys(List<Redis_KeyServerIndexDomain> list) {
		setKeys(list,MonitorExcepRedisPoolThread.getDEFAULT_EXPIRE_SECONDS());
	}
	...
	public void set(String key, Object value, Long expireTime) {
		...
		try {
			...
			redisPool.returnBrokenResource(jedis);
			LOG.error(redisPool.getExceptionMsg("operator[set] jedis is error!"), e);
			throw new RuntimeException("operator[set] jedis is error!"+e.getMessage());
		}
	}
	//批量插入keys add by sunJiaming 20140417
	public void setKeys(List<Redis_KeyServerIndexDomain> list, Long expireTime) {
		Jedis jedis = redisPool.getResource();
		if (jedis == null) {
			throw new RuntimeException("获取Jedis连接失败!");
		}
		try {
			Pipeline pl = jedis.pipelined();
			for(Redis_KeyServerIndexDomain doamin : list){
				byte[] byte_key = SerializeUtil.serializeKey(doamin.getKey());
				byte[] storeValue = SerializeUtil.serialize(doamin.getValue());
				pl.set(byte_key, storeValue);
				if (expireTime > 0) {
					pl.expire(byte_key, expireTime.intValue());
				}
			...
			pl.sync();
			redisPool.returnResource(jedis);
			LOG.debug("[DEBUG]set keys. set over!");
		} catch (Exception e) {
			e.printStackTrace();
			redisPool.returnBrokenResource(jedis);
			LOG.error(redisPool.getExceptionMsg("operator[set] jedis is error!"), e);
...
//		System.out.println(key+"==:=="+jedis.getClient().getPort());
		...
		try {
			...
			throw new RuntimeException("operator[get] jedis is error!");
		}
	}
...
//		System.out.println(key+"==:=="+jedis.getClient().getPort());
		...
		try {
			byte[] byte_key = SerializeUtil.serializeKey(key);
			byte[] storeValue = SerializeUtil.serialize(value);
			byte[] returnValue = jedis.getSet(byte_key, storeValue);
			redisPool.returnResource(jedis);
			Object result = SerializeUtil.unSerialize(returnValue);
			return result;
		} catch (Exception e) {
			redisPool.returnBrokenResource(jedis);
			LOG.error(redisPool.getExceptionMsg("operator[get] jedis is error!"), e);
