...
//		jpc.setMaxActive(Integer.valueOf(redis_Server_instance.getServerAttrMap().get("maxActive")));
		jpc.setMaxTotal(Integer.valueOf(redis_Server_instance.getServerAttrMap().get("maxActive")));
		...
		jpc.setMaxWaitMillis(Long.valueOf(redis_Server_instance.getServerAttrMap().get("maxWait")));
...
//				}
			...
			if(this.isRead_only()){
				...
				if(!flag){
					//add by sunJiaming 20141015
					this.getJedisPool().destroy();
					this.setJedisPool(createJedisPool(getRedis_Server_instanceDomain()));
